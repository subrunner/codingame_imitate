...
(function () {
   ...
   */
	...
	CODE.SKILLLEVELS = ["Starting", "First Steps", "Easy", "Medium", "Hard"];
 ...
 */
	function showSuccessfulTestcases(showBlindtestResult = false) {
		...
		if (!CODE.C.isBlindTests) {
			...
			if (successes === numberCases) {
				window.showPopup("Test Success", "You have solved all testcases. Now it is time to test your code by releasing it.<p>Press the 'Release' button to see how it fares in the wild!");
			}
			...
			if (releaseCases === successes) {
				window.showPopup("SUCCESS", '<p>Alle ' + releaseCases + ' Releasetests wurden erfolgreich durchgeführt.<div class="success">' + CODE.G.successMessage + "</div>");
				...
				window.showPopup(successPercentage + "% Success-rate", "Only " + successPercentage + "% of the cases were solved:" + cases.join(""));
     ...
     */
		...
		CODE.C.successfulTestcases = CODE.G.testcases.map(() => false);
     ...
     */
		...
		if (!myCodeMirror) {
			...
			// create the editor
			myCodeMirror = window.CodeMirror(elCodearea, {
				value: codeTemplate,
				mode: "javascript",
				lineNumbers: true
			});
